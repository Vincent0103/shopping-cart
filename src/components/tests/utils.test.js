...
describe("toUrlSafe", () => {
  it("converts a string to a url safe text", () => {
    let strings = ["Electronics", "mens clothing", "SanDisk SSD PLUS"];
  ...
  it("handles a text that is itself an url", () => {
    ...
    expect(
      toUrlSafe("http://localhost:5173/product/fjallraven---foldsack-no-1-"),
...
describe("calculateTotal", () => {
  ...
  it("throws an error if the prices contain a badly formatted price", () => {
    ...
    expect(() => calculateTotal(badPrices)).toThrow(
      "Make sure 1736.001$ is correctly formatted in a float/int type (e.g. 49.99)",
    );
  });
